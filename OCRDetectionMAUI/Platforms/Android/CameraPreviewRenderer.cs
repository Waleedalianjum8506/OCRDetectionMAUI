...
namespace BarcodeQrScanner.Platforms.Android
{
    public class CameraPreviewRenderer : FrameLayout, IVisualElementRenderer, IViewRenderer, TextureView.ISurfaceTextureListener, Camera.IPreviewCallback, Handler.ICallback
    {
        ...
        public bool HandleMessage(Message msg)
        {
            if (msg.What == 100)
            {
                ...
                if (image != null)
                {
                    ...
                    output = ConvertYuvImageToJpegByteArray(image,previewWidth,previewHeight);
                }
        ...
        public byte[] ConvertYuvImageToJpegByteArray(YuvImage yuvImage, int width, int height)
        {
            try
            {
                using (MemoryStream outStream = new MemoryStream())
                {
                    ...
                    byte[] jpegByteArray = outStream.ToArray();
